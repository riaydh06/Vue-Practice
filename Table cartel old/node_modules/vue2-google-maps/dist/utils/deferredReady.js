...
function runHooks(vm) {
  ...
  var beforePromise = vm.beforeDeferredReady ? typeof vm.beforeDeferredReady.then === 'function' ? vm.beforeDeferredReady : _Promise.all(vm.beforeDeferredReady) : _Promise.resolve(null);

  beforePromise.then(function () {
    if (typeof hooks === 'function') {
      hooks = [hooks];
    }
    return _Promise.all(hooks.map(function (x) {
      try {
        return x.apply(vm);
      } catch (err) {
...
export var DeferredReadyMixin = {
  ...
  created: function created() {
    ...
    this.$deferredReadyPromise = new _Promise(function (resolve, reject) {
      // eslint-disable-line no-unused-vars
